...
deaf_interpreter_data = open("converteddata/partial_deaf_interpreter_data_{}.csv".format(todaydate), "w", newline='', encoding='utf-8')
fields = ['Name', 'Language', 'Current Certifications', 'Location', 'Phone', 'Email', 'Website', 'Past Credentials']
deaf_interpreter_writer = csv.DictWriter(deaf_interpreter_data, fieldnames=fields)
# write headers
deaf_interpreter_writer.writeheader()
...
# choose deafinterpreter selection
...
while(True):
    ...
    while (pages <= page_number):
        ...
        while (number <= 5):
            ...
            # add to csv file
            deaf_interpreter_writer.writerow({'Name' : name, 'Language': language, 'Current Certifications' : current_certs, 'Location' : location, 'Phone' : contact_number, 'Email' : email, 'Website': website,  'Past Credentials': past_cred})
            # close the box

...
translater_data = open("translatorconvert/partialtranslator_data_{}.csv".format(todaydate), "w", newline='', encoding='utf-8')
...
while(True):
    ...
    if driver.find_element_by_xpath('//*[@id="od-table-content"]').text == "No practitioner found.":
        ...
        continue
    if language_index in [13, 29, 55, 71, 123, 184, 212, 238, 262, 390]:
        language_index = language_index + 1
        continue
    # wait till data loads
    WebDriverWait(driver, delay).until(EC.invisibility_of_element_located((By.XPATH,'//*[@id="inside-content-right"]/div[1]/img')))
    ...
    while (pages <= page_number):
        ...
        while (number <= 5):
            ...
            # add to csv file
            translater_writer.writerow({'Name' : name, 'Language': language, 'Current Certifications' : current_certs, 'Location' : location, 'Phone' : contact_number, 'Email' : email, 'Website': website,  'Past Credentials': past_cred})
